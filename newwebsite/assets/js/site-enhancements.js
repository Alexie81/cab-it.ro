(function () {
  ...
  /* Google Analytics + Google Ads configuration migrated from the previous website. */
  if (!window.__cabitGoogleTagLoaded) {
    window.__cabitGoogleTagLoaded = true;
    window.dataLayer = window.dataLayer || [];
    window.gtag = window.gtag || function () { window.dataLayer.push(arguments); };
    window.gtag("js", new Date());
    window.gtag("config", "G-QPKXFL2GW9");
    window.gtag("config", "AW-11509007584");
    window.gtag("config", "AW-11509007584/6OY0CIeMyfsZEOCJ9u8q", {
      phone_conversion_number: "+40 771 532 949"
    });
    var googleTagScript = document.createElement("script");
    googleTagScript.async = true;
    googleTagScript.src = "https://www.googletagmanager.com/gtag/js?id=G-QPKXFL2GW9";
    ...
    window.gtag_report_conversion = function (url) {
      ...
      window.gtag("event", "conversion", {
        send_to: "AW-11509007584/GqVQCOudyvsZEOCJ9u8q",
    ...
    '<div id="cabit-global-drawer" class="cabit-global-drawer" data-cabit-global-drawer hidden aria-hidden="true">' +
      ...
      '<aside class="cabit-global-drawer__panel" role="dialog" aria-modal="true" aria-labelledby="cabit-drawer-title">' +
        ...
        '<div class="cabit-global-drawer__body"><div class="cabit-global-drawer__intro"><span class="cabit-global-drawer__eyebrow">Partenerul tău digital</span><h2 id="cabit-drawer-title">Construim creștere online, clar și măsurabil.</h2><p>Spune-ne ce vrei să obții, iar noi transformăm obiectivul într-un plan ușor de înțeles.</p></div>' +
          ...
          '<div class="cabit-global-drawer__social" role="group" aria-label="Rețele sociale">' + globalSocialLinks + '</div>' +
        '</div>' +
      '</aside>' +
    '</div>';
  ...
  if (servicePage) {
    ...
    var serviceUiIcons = [
      '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/><path d="M12 2v3m10 7h-3M12 22v-3M2 12h3"/></svg>',
      '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 15 8-11 8 11-8 5Z"/><path d="m8 14 4 2 4-2M12 4v12"/></svg>',
      '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 19V9m6 10V5m6 14v-7m4 7H2"/><path d="m4 8 6-4 6 4 5-5"/></svg>'
    ];
    var benefitGrid = document.querySelector(".cabit-card-grid");
    if (benefitGrid) {
      benefitGrid.classList.add("cabit-service-benefit-grid");
      benefitGrid.querySelectorAll(":scope > .cabit-content-card").forEach(function (card, index) {
        card.classList.add("cabit-service-benefit-card");
        if (!card.querySelector(".cabit-service-benefit-icon")) {
          card.insertAdjacentHTML(
            "afterbegin",
            '<span class="cabit-service-benefit-icon is-' + ((index % 3) + 1) + '">' + serviceUiIcons[index % serviceUiIcons.length] + '</span><span class="cabit-service-benefit-index">0' + (index + 1) + "</span>"
          );
        }
      });
    }

    var caseLayout = document.querySelector(".cabit-case-layout");
    if (caseLayout) {
      caseLayout.classList.add("cabit-service-case");
      var caseAside = caseLayout.querySelector("aside");
      if (caseAside) {
        var serviceExamples = {
          website: { title: "Maison Bébé", copy: "Website e-commerce premium, responsive și ușor de administrat, construit în jurul identității brandului.", tags: ["E-commerce", "Responsive", "Administrabil"], href: "/portofoliu/maison-bebe/", action: "Vezi studiul de caz" },
          seo: { title: "Auto La Domiciliu", copy: "Structură SEO și pagini locale care conectează serviciile auto cu expresiile căutate de clienți.", tags: ["SEO local", "Conținut", "Măsurare"], href: "/portofoliu/auto-la-domiciliu/", action: "Vezi studiul de caz" },
          ads: { title: "Traffic PUB", copy: "Campanii și optimizări orientate spre rezervări, cu mesaje adaptate după rezultatele reale.", tags: ["Google Ads", "Tracking", "Rezervări"], href: "/portofoliu/traffic-restaurant/", action: "Vezi studiul de caz" },
          social: { title: "Spălătoria Ozana", copy: "Conținut și administrare Facebook cu interacțiune constantă, conectate la un website modern și rapid.", tags: ["Facebook", "Conținut", "Interacțiuni"], href: "/portofoliu/", action: "Vezi proiectele" },
          automation: { title: "Flux automat de calificare", copy: "Cererea intră din formular, este verificată, clasificată și trimisă instant echipei cu toate datele utile.", tags: ["Cerere", "Calificare", "Notificare"], href: "/contact/", action: "Discută fluxul tău" },
          conversion: { title: "Traseu optimizat pentru cereri", copy: "Mesaj clar, CTA vizibil și formular scurt, urmărite până la acțiunea care contează pentru afacere.", tags: ["UX", "CTA", "Conversii"], href: "/#audit", action: "Cere auditul gratuit" },
          analysis: { title: "Audit digital complet", copy: "Viteză, mobil, SEO tehnic, mesaj, securitate și conversii verificate într-o singură analiză.", tags: ["6 zone", "30 minute", "100% gratuit"], href: "/#audit", action: "Cere auditul gratuit" },
          local: { title: "Vizibilitate locală măsurabilă", copy: "Website, profil Google și pagini locale aliniate pentru căutările relevante din București și Ilfov.", tags: ["Google Maps", "București", "Apeluri"], href: "/portofoliu/auto-la-domiciliu/", action: "Vezi exemplul" }
        };
        var appliedExample = serviceExamples[serviceKey] || serviceExamples.analysis;
        caseAside.classList.add("cabit-service-case-preview");
        caseAside.innerHTML =
          '<span class="cabit-service-case-preview__icon" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="13" rx="2"/><path d="M8 21h8M12 17v4M7 9h4m-4 3h7"/></svg></span>' +
          '<span class="cabit-service-case-preview__label">Exemplu aplicat</span>' +
          '<strong class="cabit-service-case-preview__title">' + appliedExample.title + "</strong>" +
          '<p class="cabit-service-case-preview__copy">' + appliedExample.copy + "</p>" +
          '<div class="cabit-service-case-preview__tags">' + appliedExample.tags.map(function (tag) { return "<span>" + tag + "</span>"; }).join("") + "</div>" +
          '<a class="cabit-service-case-preview__link" href="' + siteUrl(appliedExample.href) + '">' + appliedExample.action + " <span>→</span></a>";
      }
    }

    var serviceExplainer = document.querySelector(".cabit-service-explainer");
    if (serviceExplainer) {
      serviceExplainer.classList.add("cabit-service-timeline-section");
      var serviceSteps = serviceExplainer.querySelectorAll(".cabit-service-steps > li");
      serviceSteps.forEach(function (step, index) {
        if (!step.querySelector(".cabit-service-step-icon")) {
          step.insertAdjacentHTML(
            "afterbegin",
            '<i class="cabit-service-step-icon is-' + ((index % 3) + 1) + '">' + serviceUiIcons[index % serviceUiIcons.length] + "</i>"
          );
        }
      });
    }
    ...
    var faqIcons = [
      '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M9.8 9a2.4 2.4 0 1 1 3.8 2c-1 .7-1.6 1.2-1.6 2.5M12 17h.01"/></svg>',
      '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>',
      '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 19h14V7H5zM8 4v3m8-3v3M5 10h14"/><path d="m9 15 2 2 4-4"/></svg>'
    ];
    document.querySelectorAll(".cabit-service-faq details").forEach(function (details, index) {
      var summary = details.querySelector("summary");
      if (!summary) return;
      var toggle = summary.querySelector(":scope > span:last-child");
      if (toggle) toggle.classList.add("cabit-faq-toggle");
      if (!summary.querySelector(".cabit-faq-question-icon")) {
        summary.insertAdjacentHTML("afterbegin", '<i class="cabit-faq-question-icon is-' + ((index % 3) + 1) + '">' + faqIcons[index % faqIcons.length] + "</i>");
      }
    });

    document.querySelectorAll(".cabit-service-related > a").forEach(function (link, index) {
      if (!link.querySelector(".cabit-related-icon")) {
        link.insertAdjacentHTML("afterbegin", '<i class="cabit-related-icon is-' + ((index % 3) + 1) + '">' + serviceUiIcons[index % serviceUiIcons.length] + "</i>");
        link.insertAdjacentHTML("beforeend", '<i class="cabit-related-arrow" aria-hidden="true">→</i>');
      }
    });

    var serviceSections = document.querySelectorAll("main > .cabit-content-section");
    var finalServiceSection = serviceSections.length ? serviceSections[serviceSections.length - 1] : null;
    if (finalServiceSection && !finalServiceSection.classList.contains("cabit-service-faq-section") && !finalServiceSection.querySelector(".cabit-google-partner-feature")) {
      finalServiceSection.classList.add("cabit-service-final-cta");
      var finalServiceContainer = finalServiceSection.querySelector(".container");
      if (finalServiceContainer && !finalServiceContainer.querySelector(".cabit-service-final-cta__visual")) {
        finalServiceContainer.insertAdjacentHTML(
          "beforeend",
          '<span class="cabit-service-final-cta__visual" aria-hidden="true"><i></i><i></i><b>→</b></span>'
        );
      }
    }
    var googlePartnerFeature = document.querySelector(".cabit-google-partner-feature");
    if (googlePartnerFeature) {
      googlePartnerFeature.closest(".cabit-content-section").classList.add("cabit-google-partner-section");
    }
  }
    ...
    simpleFooter.outerHTML =
      '<footer class="p-relative include-bg cabit-shared-legacy-footer" style="background-image:url(' + siteUrl("/img/footer-bg-3.jpg") + ')">' +
        '<div class="tp-footer-area-3 pt-115"><div class="tp-footer-top-widget-3 p-relative"><div class="container container-large"><div class="row">' +
          ...
          '<div class="col-xl-2 col-lg-3 col-sm-6 col-12"><div class="tp-footer-widget-3 tp-space-col-3 footer-info mb-40"><span class="tp-footer-widget__title">Contact info</span><ul><li>Sediu social: Intrarea Humulești 6A, 052034 București, România</li><li>Întâlniri online, cu programare</li><li><a href="mailto:contact@cab-it.ro">contact@cab-it.ro</a></li><li><a href="mailto:office@cab-it.ro">HR: office@cab-it.ro</a></li><li><a href="tel:+40771532949">+40 771 532 949</a></li></ul></div></div>' +
    ...
    '<footer class="cabit-global-footer" data-cabit-global-footer>' +
      '<div class="cabit-global-footer__inner">' +
        '<div class="cabit-global-footer__grid">' +
          '<div class="cabit-global-footer__brand-column"><a class="cabit-global-footer__brand" href="' + siteUrl("/") + '" aria-label="Cab-IT Expert — Acasă"><img src="' + siteUrl("/img/logo_home.png") + '" alt="Cab-IT Expert — Future is Online" width="560" height="195"></a><p>CAB-IT EXPERT SRL ajută afacerile să crească prin website-uri profesionale, SEO și campanii digitale construite pe date.</p><div class="cabit-global-footer__social" role="group" aria-label="Rețele sociale">' + globalSocialLinks + '</div></div>' +
          ...
          '<div class="cabit-global-footer__column cabit-global-footer__contact"><h2>Contact</h2><address><span>Sediu social</span><br>Intrarea Humulești 6A<br>052034 București, România</address><p><span>Întâlniri</span><strong>Online, cu programare</strong></p><a href="tel:+40771532949">+40 771 532 949</a><a href="mailto:contact@cab-it.ro">contact@cab-it.ro</a><a href="mailto:office@cab-it.ro">HR: office@cab-it.ro</a></div>' +
